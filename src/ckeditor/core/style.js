...
( function() {
	...
	CKEDITOR.style.prototype = {
		...
		checkElementSoftMatch: function(element, fullMatch) {
			var def = this._.definition;
	        ...
	        if ( typeof this.element == 'string' ? name == this.element : name in this.element ) {
	            ...
	            if ( attribs._length ) {
	                for ( var attName in attribs ) {
	                    ...
	                    // Special treatment for 'style' attribute is required.
	                    if ( attName == 'style' ? compareSoftCssText( attribs[ attName ], elementAttr ) : attribs[ attName ] == elementAttr ) {
	                        if ( !fullMatch )
	                            return true;
	                    } else if ( fullMatch ) {
	                        return false;
	...
	function compareSoftCssText( source, target ) {
        if ( typeof source == 'string' ) {
            source = CKEDITOR.tools.parseCssText( source );
        }
    ...
    function styleCompare(source, target) {
        return (source == target || source == 'inherit' || target == 'inherit');
    }

    function arrayStyleCompare(source, target) {
